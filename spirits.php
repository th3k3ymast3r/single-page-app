...
<aside role="complementary">
	<ul>
		...
		<li><?php echo '<a class="active link" href="'.$xmlData->sitedata->sitenav->category[3]->submenu[14]->url.'">'.$xmlData->sitedata->sitenav->category[3]->submenu[14]->label.'</a>';?></li>
	</ul>
</aside>
<div id="main" role="main"><?php echo $xmlData->sitedata->pages->spirits->content; ?></div>
<script>
	$(document).ready(function(){
		$('title').text('<?php echo $xmlData->sitedata->pages->spirits->title; ?>');
		$('meta[name="description"]').attr('content','<?php echo $xmlData->sitedata->pages->spirits->metadescription; ?>');
