...
<aside role="complementary">
	<ul>
		...
		<li><?php echo '<a class="active link" href="'.$xmlData->sitedata->sitenav->category[3]->submenu[12]->url.'">'.$xmlData->sitedata->sitenav->category[3]->submenu[12]->label.'</a>';?></li>
...
<div id="main" role="main"><?php echo $xmlData->sitedata->pages->rums->content; ?></div>
<script>
	$(document).ready(function(){
		$('title').text('<?php echo $xmlData->sitedata->pages->rums->title; ?>');
		$('meta[name="description"]').attr('content','<?php echo $xmlData->sitedata->pages->rums->metadescription; ?>');
