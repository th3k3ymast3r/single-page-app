...
<aside role="complementary">
	<ul>
		...
		<li><?php echo '<a class="active link" href="'.$xmlData->sitedata->sitenav->category[2]->submenu[6]->url.'">'.$xmlData->sitedata->sitenav->category[2]->submenu[6]->label.'</a>';?></li>
	</ul>
</aside>
<div id="main" role="main"><?php echo $xmlData->sitedata->pages->dessertsmenu->content; ?></div>
<script>
	$(document).ready(function(){
		$('title').text('<?php echo $xmlData->sitedata->pages->dessertsmenu->title; ?>');
		$('meta[name="description"]').attr('content','<?php echo $xmlData->sitedata->pages->dessertsmenu->metadescription; ?>');
